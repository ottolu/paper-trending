"""Run GPT-5.4 via codex exec on 20 full-text papers using v4 peer-review prompt."""
...
async def codex_exec(paper: dict) -> tuple[dict, str]:
    ...
    output_path = PROJECT / f".tmp_codex_v4_out_{paper['idx']}.json"
...
async def analyze(sem: asyncio.Semaphore, paper: dict) -> dict:
    async with sem:
        ...
        print(f"[gpt54 v4 {paper['idx']:02d}] {paper['arxiv_id']} in {elapsed}s -> "
